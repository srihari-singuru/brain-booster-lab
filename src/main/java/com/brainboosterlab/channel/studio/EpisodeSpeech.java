...
public record EpisodeSpeech(String model, String voice, List<PuzzleSpeech> puzzles) {
    ...
    public void validate(EpisodeSpec spec) {
        ...
        for (int i = 0; i < puzzles.size(); i++) {
            ...
            duration(track.questionSeconds(), "Question speech", 2, StudioRenderer.sourceDurationLimit(StudioRenderer.VISUAL_SETUP_SECONDS));
            duration(track.timerCueSeconds(), "Timer cue speech", .25, StudioRenderer.sourceDurationLimit(StudioRenderer.VISUAL_TIMER_CUE_SECONDS));
            duration(track.revealSeconds(), "Reveal speech", 2, StudioRenderer.sourceDurationLimit(StudioRenderer.VISUAL_REVEAL_SECONDS));
        }
    }
